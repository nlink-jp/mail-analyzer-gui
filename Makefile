APP_NAME    := MailAnalyzerGUI
NAME        := mail-analyzer-gui
BUNDLE_ID   := jp.nlink.mail-analyzer-gui
...
# Hardened Runtime alone suffices (the WebView JIT entitlements died with the
# Tauri implementation). Reading/writing user-selected files requires no
...
# `mail-analyzer-gui --version` must answer from PATH (brew test); without
# this symlink the executable sits unreachable inside the bundle.
BREW_BINARY     := $(NAME)
...
build-app: build
	...
	@# does NOT look here — see Sources/MailAnalyzerGUI/ResourceBundle.swift,
	@# which is what actually finds them. Guarded: the scaffold has no
	@# resource bundles yet.
