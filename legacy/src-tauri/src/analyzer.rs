...
/// Run the analyzer binary on the given file path.
pub fn run_analysis(settings: &Settings, file_path: &str) -> Result<AnalysisResult, String> {
    ...
    // Set all configured environment variables.
    ...
    let output = cmd
        .output()
        .map_err(|e| format!("Failed to execute analyzer: {}\nBinary: {}", e, settings.binary_path))?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        let stdout = String::from_utf8_lossy(&output.stdout);
        let mut msg = format!("Analyzer exited with code {}", output.status);
        if !stderr.is_empty() {
            msg.push_str(&format!("\n\nStderr:\n{}", stderr.trim()));
        }
    ...
    if stdout.trim().is_empty() {
        return Err("Analyzer returned empty output".to_string());
    }
